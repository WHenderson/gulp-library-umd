var compile, defaultOptions, dot, es, extend, fs, gutil, jsonify, namespace, offsetSourceMap, path, pipe, rename, render, templatesBase, wrap,
...
offsetSourceMap = function(file, row, col) {};
  ...
  return es.map(function(file, cb) {
    ...
    if (options.templatePath != null) {
      if (options.templateCache && (cache[options.templatePath] != null)) {
        return options.template = cache[options.templatePath];
      } else {
        return fs.readFile(options.templatePath, function(err, data) {
          var template;
          if (err != null) {
            return cb(err);
          }
